...
Status: Draft（设计审阅版）。D1–D4 产品选择已确认，维护者已请求实现；本稿先按仓库规则进入设计审阅，尚未成为可编码的 Accepted 基线。
Work: ai-daming/clickvibe#177（updatedAt 2026-09-10T17:49:22Z）
ClickVibe baseline: 82cf1bcc8a88b13c7c304eb62d44fd004ed70aea
...
建议创建普通 Harness 讨论会话，保留只读评估会话原样，不把评估 Agent 直接升级为可写 Agent。复用 #53 的工作区/会话桥，将原 Issue、报告及已确认结论带入；是否自动发送首条讨论消息须在交互确认中明确，不能把仅导航当作消息已发送。
...
正式 ADR 须明确这是对 ADR-0012 既有 unknown canonical 准入的一处限定补充；其余分支不变。只有该设计经接受、合入并重新通过 impl-gate 后才实现。
...
正式设计合入前需固定：数据 schema 与转换签名、评论 marker/完整性格式、触发与预算、会话完成接口及讨论交互；D4 不再重问。实现后工程门禁、独立 review 和 #177 人工端到端 AC 必须分别完成。人工验收 owner/触发点需最终确认，不能把设计接受记成完成。
...
当前状态：D1–D4 的产品选择已确认。技术设计继续按这些选择收口；这不等于整个 ADR 已接受或已授权实现。
...
推荐首版固定全局并发 1，FIFO 排队；相同 Work Item + basis 的多个请求合并引用，已经有效的 published 结果直接 reused。运行数、剩余请求预算和截止时间在创建 Agent 前检查；超限项目标明未运行，不把部分完成冒充整批成功。运行失败不自动多花一轮模型预算，首版由“重试”显式触发；它复用可用的来源信息，但产生新的 runId，保留原失败历史。

自动模式建议仅在所选项目列表使用期间发现缺失/过期目标时入队，页面后台定时器只查询进度，不每次启动新模型；离开页面不取消已接受请求。项目关闭页面后不维持无限全仓扫描。此触发细化仍需在最终设计说明中呈现给维护者，不悄悄扩大为 24 小时全仓服务。
...
当前技术方案已明确最小存储、串行化、取消、报告发布和复用主路径。D4 已确认并在 §9 限定授权与开发/review 输入的变化；产品选择完成不自动构成技术设计接受或 READY。

逻辑记录和转换契约见 §16；实现类型必须忠实映射，不借类型细节增加新的判定标准。所有用户已确认条款保留在决策表；未确认的推荐不得当作已授权实现。本 PR 供设计审阅，仍不执行实际评估或修改 GitHub Issue 正文。
...
正式设计 PR 需要同步 ADR 索引、架构入口，以及 ADR-0012 对 §9 限定入口的补充说明；记录对旧“所有 canonical unknown 一律拒绝”规则的精确替代，不另造第二套契约规范。
...
设计状态：可进入设计审阅，尚未标为 Accepted。产品选择 D1–D4 已记录，维护者已请求 Coding，但实施仍须等待设计基线与 impl-gate；不授权评估模型调用或重启服务。
