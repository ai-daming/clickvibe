# #170 Shell 诊断与工作区恢复：设计草案

Status: Proposed revision 2 / 待维护者接受 — 非 Accepted，不授权实现。
Date: 2026-09-09
Work: https://github.com/ai-daming/clickvibe/issues/170
Code baseline: `777364e4c95f10cce18efdff529467b9602879d6`
Inspected checkout: `5e1674a5365e129e8a704998a837bafbaae57f78`；仅比 main 多 #169 设计文件，不引用其为 Accepted 基线。

## 1. 目标、权限与决定

让命令失败能够解释；有效工作区不因控制器保存失败被删除；证据充分时恢复，持续失败有确定终点。
本轮只形成设计草案，不改业务代码、测试、配置、GitHub 或运行状态。
...
| Agent 设计建议 | 恢复额度属于一次自动运行，只有新的人为授权才重建额度；成功推进可清连续错误计数，但不能补回已消费额度 |
| 非目标 | 全局 Shell 队列、跨机器执行、v0.3 Policy/Lease/Envelope、自动清理历史现场、变更 GitHub 限流策略 |

Q1/Q2 是产品边界接受，不是对本文件新增机制的接受。缺失事实不交给用户猜测。
...
- 命令结果未知：保留 prepared，不因 null 判“尚未执行”；确认旧执行者已结束后回读。
- Git 完成、保存失败：保留现场和 prepared，不执行 force remove/branch -D；不启动 Agent。
- prepared + 可证明旧写入者结束 + Git 符合目标：补记 verified 后恢复；完全符合操作前事实时才允许重新派发。
...
候选格式：config schemaVersion=2；state marker schemaVersion=2、generation=v0.2-recovery-1；原 v0.2 journal 保留原样。新增本次离线升级 journal，schemaVersion=1，独立描述 prepared/config-written/marker-written/verified 状态和原/目标文件哈希、备份清单、授权 fingerprint。
这不是借用 ADR-0009 授权；必须由接受本设计的新 ADR 明确授权此次格式与升级协议，才可实现或执行。
...
候选方案保留现场与有限重试：可自愈，但增加持久准备状态、部署兼容成本，且未知旧子进程会保守停机。
...
| 状态与兼容 | §7 已定义；离线升级、旧版拒写、零使用回滚限制需要维护者接受 | 实现前架构接受；现场执行另行授权 |
| 发布责任 | 发布操作者尚未确认；发布记录必须包含以下清单，不能把本文件视为已委派 | 发布前必须确认 owner，否则不执行升级 |

待执行发布清单（维护者决定发布时触发，执行前必须填写 owner 与日期）：
- [ ] owner 接受维护窗口，记录旧/新 artifact SHA，确认停止与禁用自动重启的方法。
...
修订记录：revision 1 的 E2/E3 机制缺口由 §4.4/§7 替代。仍未接受本草案，不能发 READY 回执。发布 owner 尚未接受任务，不能称为交接完成；这不作为声称“升级已验证”的依据。
