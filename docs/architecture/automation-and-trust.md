...
v0.1 已支持显式开启 auto-merge，但产品文档长期把“merge 必须人点”写成永久原则，这是过期设计。v0.2 的正式原则是：
...
Coding 和 Review 都处于交付循环内部：Coding 的激励是通过 Review，Review 的激励是继续发现问题。自动化不能指望二者自行发现“方法本身没有收敛”，因此每轮 Review 落盘后由 Controller 的纯规则 Loop Guard 核对：

- Review 明确输出 `stop-and-redesign`：立即启动 Runtime Observer。
- 同一 CRITICAL 母题连续两轮复发：立即启动 Runtime Observer。
- 连续三轮 Review 未通过：在进入下一轮前必须启动 Runtime Observer。
- 修复 diff 连续两轮增长且高优问题集合未缩小：启动 Runtime Observer。
- 人显式要求观察：永远允许，不需要满足量化阈值。

轮数是兜底，不是“无进展”的唯一含义。具体阈值属于版本化项目策略，可在后续基于真实复盘调整；“模型不能自触发”“Observer 期间冻结同一 workflow”“判决绑定完整证据快照”是不可变边界。

一次 Runtime Observer 介入后，只允许按其唯一指令进行一个验证轮。相同母题再次出现、Observer 无法验证关键 finding、需要扩大权限/改业务合同，或介入预算耗尽时，转为 `human-required`，禁止 Coding/Review 继续互相打转。
