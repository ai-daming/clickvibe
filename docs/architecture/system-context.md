...
| Loop Guard | 根据持久化轮次证据确定性判断继续、观察或升级 | 调用模型解释问题；修改代码或事实 |
| Agent | 在 Issue 范围内编码、测试、review、解决冲突、使用必要的 git/gh 工具 | 把自己的完成声明当成系统事实 |
| Runtime Observer | 在独立 DSH 会话中审计跨轮收敛性、验证关键 finding、给出一个介入判决 | 直接修改业务代码、自动修改全局协议或授予 merge 权限 |
...
v0.2 仍是 local-first 单宿主控制面。DSH 插件只接受本机回环、同源和专用请求头；跨机器执行属于后续独立架构问题，不能通过把当前面板直接暴露到公网解决。Runtime Observer 必须由宿主侧任务驱动，使用任务专属会话；不能依赖浏览器保持打开，也不能向用户当前 Chat 隐式发送消息。
