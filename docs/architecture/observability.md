...
启用模型型 Runtime Observer 时，运行时介入至少追加以下不可变事件，不允许只在面板显示一段摘要：
...
v0.3 即使不启用模型，也必须记录 Loop Guard 的触发规则、冻结证据、暂停原因和下一步。v0.6 介入产品化只改变证据查看、指令修改和恢复入口，不改变事件权威；模型诊断仍是可选证据。
