...
| `restore-base <目标>` | 恢复基线 | 远端基线已删除时,按 workflow 冻结提交原子恢复同名分支 | 是(显式二次确认) |
...
- 写命令(`develop/review/rework/resume/restore-base/merge`)是**两阶段**的,与面板「预览 → window.confirm → 执行」同构:
...
- 一次性授权绑定精确动作 + 快照,2 分钟过期,消费即失效;篡改 context/agent 重发会被拒。
