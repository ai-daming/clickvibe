...
准备工作区的 Git 命令按类别保留固定错误文本，以及磁盘空间不足、权限不足、只读文件系统、文件不存在、打开文件过多和 I/O 错误等固定原因片段。路径与其他上下文替换为 `[details omitted]`；未知类别、未知文本和本地化输出省略，不承诺捕获任意错误。stdout/stderr 各只处理最后 4 KiB，首个被截断的残行不采纳，省略与截断分别显示。输出只帮助诊断，不能证明命令结束或授权恢复。

- [ ] 现场检查实际失败命令的安全片段、omitted/truncated 标记与面板展示；不得把“支持固定片段”记成“完整输出已保留”。

## 未决准备的人工恢复

跨 runtime 的 `dispatched` 或明确异常的 `blocked`，按 [离线人工结算规程](issue-170-manual-preparation.md)核查。新授权本身不负责清除这些记录；禁止删除 preparation、直接改 verified 或恢复旧 workflow 来绕过门禁。恢复需要独立备份、结束证据、精确计划授权和发布回读。

Owner：维护者。触发点：#170 发布前核对该规程 exact-head review 与真实 Git/文件系统验证；现场出现未决准备时另行授权执行。实现/规程合入不代表已完成现场升级或验收。
