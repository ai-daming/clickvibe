/** Pure, dependency-free Markdown rendering helpers for the browser panel. */
...
export function renderInline(text: string, keyBase: string): React.ReactNode[] {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return (
        <code key={key} className="cv-md-code">
          {part.slice(1, -1)}
        </code>
      )
    }
    ...
    if (link) {
      ...
      return (
        <a key={key} className="cv-md-link" href={href} target="_blank" rel="noreferrer">
...
export function renderMarkdown(md: string): React.ReactNode[] {
  ...
  while (i < lines.length) {
    ...
    if (line.startsWith('```')) {
      ...
      i++
      push(
        <pre key={`b${key}`} className="cv-md-pre">
          <code>{buf.join('\n')}</code>
        </pre>,
      )
      continue
    ...
    if (h) {
      ...
      push(
        <H key={`b${key}`} className={`cv-md-h cv-md-h${level}`}>
    ...
    if (line.startsWith('>')) {
      ...
      push(
        <blockquote key={`b${key}`} className="cv-md-blockquote">
