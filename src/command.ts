...
const ACTIONS_REQUIRING_TARGET: readonly CommandAction[] = ['status', 'develop', 'review', 'rework', 'resume', 'merge', 'sync', 'stop']
...
export function formatProjects(projects: { repoKey: string; path: string; available: boolean }[]): string {
  if (projects.length === 0) return '尚未配置任何项目。请在 ~/.clickvibe/config.yaml 的 repos 中添加 owner/repo → 本机路径。'
  return ['已配置的项目:', ...projects.map((project) =>
    `- ${project.repoKey} → ${project.path}${project.available ? '' : '(路径不可用)'}`)].join('\n')
}
...
export function formatStatus(workflow: CommandStatusWorkflow | null, issueNumber: string): string {
  ...
  const lines = [
    `#${issueNumber} · ${status}`,
    `- 分支:${workflow.branch}(HEAD ${derived?.head ?? '未知'})`,
  ]
  if (workflow.prNumber) lines.push(`- PR:#${workflow.prNumber}`)
  if (workflow.reviewResult) {
    const verdict = workflow.reviewResult.passed
      ? derived?.verdictCurrent ? `通过,结论有效(${workflow.reviewResult.issues.length} 条遗留为 0)` : '通过,但结论已过期(需重新 review)'
  ...
  if (derived) {
    ...
    lines.push(`- 基线对比:${derived.mergeConflict ? '存在未解决的合并冲突' : derived.needsSync ? `${aheadBehind},需要同步` : aheadBehind}`)
  }
