...
export function aggregateRepositoryFreshness(
  freshnesses: RepositoryFreshness[],
): AggregatedRepositoryFreshness | null {
  ...
  return {
    ...
    lastSuccessAt: successful.length === 0
      ? null
      : Math.min(...successful.map((value) => value.lastSuccessAt as number)),
