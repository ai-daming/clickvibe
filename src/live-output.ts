import type { AgentKind } from './agent-stream.ts'
...
export function tokenUsage(value: unknown): TokenUsage | undefined {
  ...
  const cachedInputTokens = finiteToken(record.cached_input_tokens ?? record.cache_read_input_tokens ?? record.cachedInputTokens)
  ...
  if (inputTokens === undefined && cachedInputTokens === undefined && outputTokens === undefined && totalTokens === undefined) return undefined
...
export function encodeLiveLogEvent(event: LiveLogEvent): string {
  const bounded = event.text.length > EVENT_TEXT_MAX_CHARS
