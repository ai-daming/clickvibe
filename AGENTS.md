...
4. **对外契约默认不变**:`/clickvibe/api/*` method（以 `src/index.ts` 的 `knownMethods` 为准；ADR-0020 增量授权 `assessment` method 与 `assess` 文本命令）、响应形状、`~/.clickvibe/state/` 当前代次格式、agent 两阶段授权命令(预览→一次性 2 分钟授权→执行)与文本命令语法。只有 Accepted ADR + 显式 preview/authorization/backup/recovery/read-back 升级协议可以授权持久化代次切换；ADR-0009 仅授权一次 v0.1→v0.2 clean break，不授权普通功能 PR 随意改格式或长期双写。
